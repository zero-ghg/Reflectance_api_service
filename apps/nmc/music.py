...
def _query_file_list_by_range(client, user_id, pwd, interface_id, base_params, start_time: str, end_time: str):
    ...
    - timerange 使用格林威治时间；
    - getSevpEleByTimeRange 返回的是 JSON 数据，不是文件列表。
    """
    params = dict(base_params)
    params["timeRange"] = f"[{start_time},{end_time}]"
    params["timerange"] = f"[{start_time},{end_time}]"

    ret = client.callAPI(user_id, pwd, interface_id, params)
    if ret.request.errorCode != 0:
        raise RuntimeError(f"MUSIC 接口返回失败: {ret.request.errorMessage}")

    if hasattr(ret, "data") and isinstance(ret.data, dict):
        return ret.data.get("DS", []) or []
    if hasattr(ret, "DS"):
        return ret.DS or []
    if hasattr(ret, "result") and isinstance(ret.result, dict):
        return ret.result.get("DS", []) or []
    return []
