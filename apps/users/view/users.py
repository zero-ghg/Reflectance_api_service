...
def build_captcha_image_base64(captcha_code):
    ...
    for index, char in enumerate(captcha_code):
        x = 14 + index * 31 + random.randint(-1, 1)
...
class UserInfoLoginView(APIView):
    def post(self, request):
        ...
        if not captcha_id:
            return Response({'code': 400, 'msg': "缺少captcha_id参数"})

        # 校验验证码
        captcha_error = check_captcha(captcha_id, captcha)
        if captcha_error:
...
class UserTokenRefreshView(APIView):
    def post(self, request):
        ...
        if not refresh_token:
            return Response({"code": 400, "msg": "缺少参数 refresh"})
