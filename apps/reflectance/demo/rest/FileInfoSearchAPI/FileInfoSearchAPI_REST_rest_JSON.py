...
def json_data():
    ...
    # 获取dic数据fieldNames及对应数据
    if res['returnCode'] == str(0):
        fieldNames=res['fieldNames']
        dataSet=res['DS']
        #分割fieldNames
        fieldNames_list=fieldNames.split(',')
    # print(fieldNames_list)
    # 获取数据并输出到txt
        with open('outputdata_json.txt','w',encoding='utf-8') as file:
            ...
            for data in dataSet:
                for fieldName in fieldNames_list:
                    file.write(data[fieldName])
                    file.write('\t'*2)
                    # file.write('\t'+' '*abs(len(fieldName)-len(data[fieldName])))
                file.write('\n')
            file.close()
    # print(json_data)
